function plot_cos_fwderr_double(condest1u, perm, fwderr_cosm, fwderr_cosm_adv,...
                            fwderr_cosm_exp, fwderr_cosm_mp, fwderr_cosm_mp_s,...
                            fwderr_cosm_tay, fwderr_cosm_pol, prec, ymin, ymax,...
                            Tcolors, color_cond, Tmarkers, ls_cond, msize, lw, lw_cond,...
                            print_yticks, print_legend, legend_perm, png_out, tikz_out)
% This function generates the plots of the errors in the computed cosine 
% of the algorithms in double precision. 
%
...
% [YMIN, YMAX]: range of the errors in the plots
    ...
    fwderr_cosm(fwderr_cosm > ymax) = ymax;
    ...
    fwderr_cosm_tay(fwderr_cosm_tay > ymax) = ymax;
    fwderr_cosm_pol(fwderr_cosm_pol > ymax) = ymax;
    
    fwderr_cosm(fwderr_cosm < ymin) = ymin;
    ...
    fwderr_cosm_tay(fwderr_cosm_tay < ymin) = ymin;
    fwderr_cosm_pol(fwderr_cosm_pol < ymin) = ymin;
    ...
    mp_semilogy(mats, fwderr_cosm_tay(perm),...
                [], 0, Tmarkers{1}, 'Color', Tcolors(1,:), 'MarkerSize', msize, 'Linewidth', lw);
    mp_semilogy(mats, fwderr_cosm_pol(perm),...
    ...
    mp_semilogy(mats, fwderr_cosm(perm),...
                [], 0, Tmarkers{5}, 'Color', Tcolors(5,:), 'MarkerSize', msize, 'Linewidth', lw);  
    mp_semilogy(mats, fwderr_cosm_adv(perm),...
                [], 0, Tmarkers{6}, 'Color', Tcolors(6,:), 'MarkerSize', msize, 'Linewidth', lw);
    mp_semilogy(mats, fwderr_cosm_mp_s(perm),...
                [], 0, Tmarkers{7}, 'Color', Tcolors(7,:), 'MarkerSize', msize, 'Linewidth', lw);
...
%     set(gca,'XTick',0)
    ...
    if(print_legend)
        ...
        methods_names = {'$\kappa_{\mbox{cos}}(A)u$', 'cosm\_tay',...
            'cosm\_pol', 'cosm\_mp', 'cosm\_exp', 'cosm', 'cosm\_adv', 'cosm\_mp\_s'};
